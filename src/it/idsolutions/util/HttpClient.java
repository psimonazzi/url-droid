...
/**
 * A wrapper around HttpURLConnection, which is the API to make HTTP requests on Android officially recommended by Google.<br/>
 * Supports the JSON format for serializing/deserializing content, Gzip compression, Basic auth, HTTPS, HTTP and SOCKS proxies.<br/>
 * Each request should use its own instance of this class, unless all settings are exactly the same.
 * 
 * @author ps
 */
public class HttpClient {
	...
	/**
     * Returns a new instance bound to the specified URL.
     * The URL must be already encoded (according to RFC3986 or the obsoleted RFC2396
     * for the path part and the application/x-www-form-urlencoded format for
     * the query part).<br/>
	 * The URL can contain path params, which can then be set using #addPathParam(String,String).
	 * Path params are specified as '{name}', i.e.: <code>'/resource/{id}/1'</code>.
     * 
     * @param url 
     */
    ...
    private HttpClient execute(String method) {
        ...
        if (pathParams != null) {
            for (Map.Entry<String, String> e : pathParams.entrySet()) {
                actualUrl = actualUrl.replace("{" + e.getKey() + "}", e.getValue());
            }
        }
        ...
        if (user != null && !user.equals("") && password != null && !password.equals("")) {
        	String base64Encoded = new String(Base64.encode(new String(user + ":" + password).getBytes(), Base64.DEFAULT)).trim();
        ...
        // Use proxy if needed
        // By default, HttpURLConnection class will connect directly to the origin server (RFC2616).
        // TODO Both HTTP and SOCKS proxies are supported, need to choose
        if (proxy == null) {
            String proxyHost = System.getProperty("http.proxyHost");
            String proxyPortString = System.getProperty("http.proxyPort");
            if (proxyHost != null && !proxyHost.equals("") && proxyPortString != null && !proxyPortString.equals("")) {
        	    //client.getHostConfiguration().setProxy(proxyHost, proxyPort);
        	    //Settings.System.putString(getContentResolver(), 
        	    //		Settings.System.HTTP_PROXY, proxyHost + ":" + proxyPortString);
        ...
        if (proxy != null) {
            ...
            if (proxyUser != null && !proxyUser.equals("") && proxyPassword != null && !proxyPassword.equals("")) {
            	String base64Encoded = new String(Base64.encode(new String(proxyUser + ":" + proxyPassword).getBytes(), Base64.DEFAULT)).trim();
        ...
        try {
			...
			// HTTP standard does not forbid to send a body entity with the GET method,
			// nor query params with the POST/PUT methods, but we try to follow best practices here
			// and enforce sending query params in the body for POST/PUT requests.
            if (entity != null && !entity.equals("") &&
                ...
                // this opens a connection, then sends POST & headers, then writes body entity
                conn.getOutputStream().write(payload);
                //responseCode = conn.getResponseCode();
            }
            
			// Try to get response content, if any
            // In case of POST/PUT the connection is already open, otherwise it will be opened here
            ...
            if (content != null) {
                if (deserializedResponseType != null) {
                    ...
                    try {
                    	...
                    	//Log.e("HTTP CLIENT " + VERSION, "Cannot deserialize response. ", ex);
                        throw new RuntimeException(ex);
                    }
                }
                else {
        	...
        	//Log.e("HTTP CLIENT " + VERSION, "Cannot execute request for url " + url + " ", ex);
            throw new RuntimeException(ex);
        } 
        finally {
            if (conn != null)
     ...
     * Used for GET requests. Standard does not forbid use with POST or PUT requests,
     * but it's not best practice.
     * 
     * @param name Param name
     * @param value Param value, which will be encoded as application/x-www-form-urlencoded with charset UTF-8
     * @return 
     */
     ...
     * Used for POST or PUT requests. Standard does not forbid use with GET requests,
	 * but it's not best practice.
     * 
     * @param name Param name
     * @param value Param value, which will be encoded as application/x-www-form-urlencoded with charset UTF-8
     ...
     * The URL path will be modified by sustituting the param name with its value.
     * 
     * @param name Param name, specified in the path as '{name}'
     * @param value Param value, which will be encoded according to RFC3986 (which obsoletes RFC2396) with charset UTF-8 
     * @return 
     */
    public final HttpClient addPathParam(String name, String value) {
        ...
        try {
        	pathParams.put(name, new URI(null, null, value, null).toASCIIString());
        } catch (Exception ex) {
            // We use utf-8 hardcoded, should never throw
        	throw new RuntimeException(ex);
        }
	...
	/**
     * Add a request header. If the header was already set it will be overwritten.
     * 
	...
	/**
     * Set the request entity for requests of type application/x-www-form-urlencoded.
     * 
     * @param data Entity as string, which will be encoded as application/x-www-form-urlencoded with charset UTF-8
     * @return 
     */
    public final HttpClient entityUrlEncode(String data) {
        // Need to encode the entity only if content-type is application/x-www-form-urlencoded
     ...
     * This method is used for sending data serialized as JSON, XML, etc. To send a query string in the request body, use #entityUrlEncode(String).
     * 
     * @param data Entity as string
     * @return 
     */
     ...
     * 
     * @param data Entity object, which will be serialized as JSON
     * @return 
     * @throws RuntimeException When the given object cannot be serialized
     */
     ...
     * 
     * @param data Entity object, which will be serialized as JSON
     * @param type Object type, used as an hint for the serializer. This may be needed when sending bare generics collections, i.e. <code>new TypeToken<Collection<myDataType>>(){}.getType()</code>
     * @return 
     * @throws RuntimeException When the given object cannot be serialized
     */
    ...
    /**
     * Set the 'Accept' request header, which specifies the expected type of the response content.
     * 
    ...
    /**
     * Set the 'Content-Type' request header, which specifies the type of the request entity.
     * 
     * @param type Header value
     * @return
     */
     ...
     * In the special case that the type is JSONObject.class, Douglas Crockford's JSON deserializer will be used. In all other cases the GSON deserializer will be used.
     * 
     * @param type Object type, used as an hint for the deserializer. This may be needed with bare generics collections, i.e. <code>new TypeToken<Collection<myDataType>>(){}.getType()</code>
     * @return
     */
	...
	/**
     * Specify that no exceptions be raised in case the response HTTP status is an error (i.e. it's not 2XX).
     * 
     * @return
     */
     ...
     * Credentials can also be specified as system properties, but this method will override them.<br/>
     * If credentials are not set, the request will not enable authorization
     * 
     * @param user User (can also be specified as system property 'http.user')
     * @param password Password (can also be specified as system property 'http.password')
     * @return
     */
     ...
     * A proxy will also be used if the proxy host and port are specified as system properties, respectively 'http.proxyHost' and 'http.proxyPort'. In this case the proxy will be of type HTTP. If a proxy is specified in this way, this method will override it.<br/>
     * If the proxy requires Basic authorization, the credentials can also be set here, otherwise leave them null.
     * Credentials can also be specified as system properties, but this method will override them.<br/>
     * 
     * @param proxy Proxy
     * @param proxyUser User (can also be specified as system property 'http.proxyUser')
     * @param proxyPassword Password (can also be specified as system property 'http.proxyPassword')
     * @return
     */
    ...
    /**
     * Deserialize the given JSON string. Either Douglas Crockford's JSON or GSON will be used according to the specified return type.
     *
     */
    private Object deserialize(String content) throws Exception {
    	if (deserializedResponseType == JSONObject.class) {
	        ...
	        // Deserialize dates as Unix timestamp notation, for interoperability with Jackson-RS
    ...
    private String serialize(Object content, Type type) throws Exception {
        // Serialize dates as Unix timestamp notation, for interoperability with Jackson-RS
        Gson gson = new GsonBuilder()
                //.serializeNulls()
     ...
     * This method must be called after the request has been executed.
     * 
     * @return The response content as a deserialized object of the type specified by #returnType(Type), or the raw string
     */
	...
	/**
     * Returns the HTTP reason phrase of the response, which is a textual description of the status code.<br/>
     * This method must be called after the request has been executed.
     * 
     * @return
     */
	...
	/**
     * Read the response content in a string. This method tries to decode the content according to the specified encoding, which is UTF-8 by default.
     * 
     * @return
